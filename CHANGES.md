...
- **Abenteuerpunkte & Stufenaufstieg:** Der Charakterbogen hat jetzt einen Bereich "Stufenaufstieg" mit einem Abenteuerpunkte-Konto (manuell per +/- eintragbar) und zwei unabhängigen "Steigern"-Buttons für Heldenkarte und Talentkarte (Kosten: je 3 Abenteuerpunkte, maximal Stufe III). Ein Dialog zeigt vorher genau, welche Werte sich ändern ("Nahkampf 14 → 16" usw.) und muss bestätigt werden, bevor etwas geschrieben wird. Verfügbar für die 12 Schnellstarter-Helden aus "Kelche der Macht". Eine Steigerung lässt sich über einen "Zurückstufen"-Button wieder rückgängig machen (Werte auf den vorherigen Stand, Abenteuerpunkte zurück) - je Karte immer die zuletzt durchgeführte Steigerung. Steigerungen, die vor diesem Update gemacht wurden, lassen sich nicht zurückstufen (die alten Werte wurden damals noch nicht gespeichert).
...
- Aktionskarten verbessern bzw. zurückstufen konnte mit der Konsolenmeldung "The _id … already exists" fehlschlagen und legte im Heldendeck stillschweigend Kartendubletten an, statt die Karte sauber gegen die Erfahrungsschatz-Variante zu tauschen. Behoben. Hinweis: bereits vor diesem Fix entstandene Dubletten im Deck müssen einmalig von Hand entfernt werden.
