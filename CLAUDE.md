...
**"Spielmodus"/Sheet-Sperre ergänzt:** Schloss-Icon als Header-Control (`_getHeaderControls()`-Override, Action `toggleLock`) neben dem Standard-"Sheet konfigurieren"-Icon. Toggle schreibt ein Flag (`actor.flags.aventuria-helpers.locked`), das persistiert (nicht nur Render-State) und für alle Betrachter des Actors gilt. `isEditable` ist überschrieben (`super.isEditable && !locked`) – dadurch greift die Sperre automatisch überall dort, wo das Sheet bereits `editable`/`this.isEditable` prüft (alle Formularfelder, Bild-Editieren, Rich-Text-Editor, Kategorien-Toggle), ohne dass jedes Feld einzeln angefasst werden musste. Würfel-Buttons und Tab-Wechsel bleiben bewusst auch im gesperrten Zustand nutzbar (keine Werteänderung); das Schloss-Icon selbst bleibt immer klickbar, um wieder zu entsperren.

Nächster Schritt: Nutzer testet diese Korrekturen sowie die neue Sperre live, danach ggf. weiterer Feinschliff, danach Phase 3 (Macros).
